...
import { TEMPLATE_FILENAME, type AgentTemplate } from '../_types';
...
export async function handler(arg: vscode.Uri | undefined): Promise<void> {
  ...
  const name = await vscode.window.showInputBox({
    title: vscode.l10n.t('New agent template name'),
  ...
  const entryDir = path.join(targetFolder, 'templates', folderName);
