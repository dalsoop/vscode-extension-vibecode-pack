...
function resolveTargetDir(target) {
  ...
  switch (target) {
    case 'claude-global': return path.join(home, '.claude', 'skills');
    case 'codex-global':  return path.join(home, '.codex',  'skills');
    case 'claude-ws':     return wsPath && path.join(wsPath, '.claude', 'skills');
    case 'codex-ws':      return wsPath && path.join(wsPath, '.codex',  'skills');
    case 'copilot-ws':    return wsPath && path.join(wsPath, '.github', 'skills');
  }
}
...
async function runWizard() {
  ...
  const whenToUse = await vscode.window.showInputBox({ prompt: '"When to use" — situational trigger', value: 'When the user requests ...' });
...
module.exports = { runWizard, sanitizeName, TEMPLATE };
