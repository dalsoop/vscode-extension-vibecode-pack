...
//   2. package.nls.<locale>.json from each i18n/<locale>.json (ext block)
//   3. l10n/bundle.l10n.<locale>.json from each i18n/<locale>.json (runtime block)
//
// Unlike the apps-pattern sync, this extension declares contributes manually in package.json
// (it's a customEditor, not a command), so this script only touches NLS files.
//
// Usage:
...
async function main() {
  ...
  // Per-locale — `nls` block in i18n/<locale>.json maps 1:1 onto package.nls.<locale>.json keys.
  // Falls back to defaults per-key so a missing translation just shows English.
